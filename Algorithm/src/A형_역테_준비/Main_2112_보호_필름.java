...
public class Main_2112_보호_필름 {
    ...
    static boolean inspect() { // 검사
        for (int i = 0; i < W; i++) {
            int now = film[0][i] ? 1 : 0; // 현재 검사하는 세로줄의 값
            int count = 1; // 현재 검사하는 세로줄의 연속된 값의 개수

            for (int j = 1; j < D; j++) { // 세로줄 검사
                if (count == K) break; // 연속된 값의 개수가 K개이면 다음 세로줄 검사
                if (now == (film[j][i] ? 1 : 0)) count++;  // 현재 검사하는 세로줄의 값이 이전 검사한 세로줄의 값과 같으면 count++
                else {
                    now = film[j][i] ? 1 : 0; // 현재 검사하는 세로줄의 값이 이전 검사한 세로줄의 값과 다르면 now값 변경
                    count = 1; // 연속된 값의 개수 초기화
                }
            }
    ...
    static boolean dfs(int start, int k) {
        ...
        if (len == k) { // 약품을 투입할 개수만큼 약품을 투입했으면
            for (Pair i : change) { // 약품을 투입한 위치에 약품을 투입
                for (int j = 0; j < W; j++) { // 약품을 투입한 위치의 모든 세로줄에 약품을 투입
