...
public class Main_2261_가장_가까운_두_점 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Arrays.sort(pl, Comparator.comparingInt(o -> o[0]));
        bw.write(dac(0, n - 1) + "\n");
        bw.close();
    }
    ...
    static int dac(int start, int end) {
        ...
        if (end - start == 1) return getDist(pl[start], pl[end]);
        ...
        for (int i = start; i <= end; i++) {
            if ((pl[mid][0] - pl[i][0]) * (pl[mid][0] - pl[i][0]) < minDist) target_pl[t++] = i;
        }
    ...
    static void merge(int start, int mid, int end, int idx) {
        ...
        while (i <= mid && j <= end) {
            if (pl[i][idx] < pl[j][idx]) tmp[k++] = pl[i++];
            else tmp[k++] = pl[j++];

        }
        while (i <= mid) tmp[k++] = pl[i++];

        while (j <= end) tmp[k++] = pl[j++];
